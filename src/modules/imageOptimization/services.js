...
const ImageOptimizationLog = require("../../models/imageJobFail");
const ImageOldData = require("../../models/ImageOldData");
const { get, put } = require("../../utils/axiosUtils");
const { resolveProductImageUrl } = require("../../utils/urls");
const {
  resolveOptimizeFormat,
  mapFrontendQualityToSharpQuality,
...
exports.resolveOptimizeFormat = resolveOptimizeFormat;
...
/** @deprecated Use getOptimizationTypeFromFrontendQuality */
exports.mapImageQualityToLevel = (imageQuality) => {
...
exports.getOptimizationTypeFromFrontendQuality = exports.mapImageQualityToLevel;
...
exports.updateBigCommerceProductImageMetadata = async ({
  storeHash,
  productId,
  imageId,
  accessToken,
...
function normalizeJobType(jobType) {
  ...
  return JOB_TYPES.has(value) ? value : null;
}
...
exports.createBulkOptimizationJob = async ({
  ...
  if (!validJobType) {
    return {
      error: `Invalid job_type "${jobType}". Allowed: single, bulk, checkBox, webhook, reoptimize`,
      jobUuid: null,
      doc: null,
    };
  }
...
exports.writeOptimizationLog = async ({
  jobUuid,
  storeHash,
  jobType,
  type: legacyJobType,
  ...
  const validJobType = normalizeJobType(jobType || legacyJobType) || "bulk";
  ...
  try {
    ...
    console.error("[writeOptimizationLog]", err.message);
...
exports.recordOptimizationJobImageResult = async ({
  ...
  try {
    ...
    if (!job) {
      return { error: "Optimization job not found in database", job: null };
    }
    ...
    const logWrites = [
      ImageOptimizationLog.create({
        ...
        message: success
          ...
          : errorMessage || "Image optimization failed",
        meta: success ? {} : { error: errorMessage },
      }),
    ];
