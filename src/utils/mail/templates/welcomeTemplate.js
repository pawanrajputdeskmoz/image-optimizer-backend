...
/**
 * Builds the welcome email sent after a store installs the app.
 *
 * @param {Object} vars
 * @param {string} [vars.storeName]    Store/merchant display name.
 * @param {string} [vars.planName]     Assigned plan name (e.g. "Free").
 ...
 */
function welcomeTemplate(vars = {}) {
  const { storeName, planName, monthlyLimit, message } = vars;
  ...
  const introMessage = escapeHtml(
    message ||
      "Thank you for installing Image Optimizer on your store. You can now optimize product, category, and brand images to improve page speed and enhance your storefront experience."
  );
                  ...
                  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;background-color:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;">
                    <tr>
                      <td style="padding:16px 20px;font-family:Arial,Helvetica,sans-serif;font-size:14px;color:#475569;">
                        ${safePlan ? `<div style="margin-bottom:8px;"><strong style="color:#0f172a;">Your plan:</strong> ${safePlan}</div>` : ""}
                        ${hasLimit ? `<div><strong style="color:#0f172a;">Monthly limit:</strong> ${monthlyLimit.toLocaleString()} images</div>` : ""}
                      </td>
                    </tr>
                  </table>
                </td>
...
<body style="margin:0;padding:0;background-color:#f1f5f9;">
          ...
          <tr>
            <td style="padding:8px 24px 28px 24px;">
              <p style="margin:0;font-family:Arial,Helvetica,sans-serif;font-size:13px;line-height:1.6;color:#64748b;">
                Open the Image Optimizer app from your BigCommerce admin panel to configure your settings and start optimizing your images.
              </p>
            </td>
          </tr>
          <tr>
  ...
  if (planName) textLines.push("", `Your plan: ${planName}`);
